...
import com.webcontent.htmlconvert.xslt.hunan;
...
public class crawlerTask extends Thread {
	...
	public void run() {
		...
		DocumentFragment df;
		String filename = "e:/pageclawer/"+url.substring(url.lastIndexOf("/"))+".xml";
		try {
			...
			String region_code = "430111";
			String phone  = xslt.getXPathContent(doc,hn.getPhone()).replaceAll("\\n", "");
	...
	public synchronized void writeDate(String name,
			...
			String pic1,String pic2,String pic3,String pic4) throws IOException
			{
				...
				double hot = 0;
				writer.write(name+","+address.replaceAll(",", " ")+","+pic+","+introduction.replaceAll(",", " ")+","+destination_type+",,,,"+phone.replaceAll(",", " ")+",,,"+hot+","+price+","+price+",,"+bus.replaceAll(",", " ")+","+car.replaceAll(",", " ")+",0,"+destination_type.replaceAll("±Í«©£∫", "").replaceAll(" ", "&")+",1,"+region_code+",,0,0,0,0,0,"+pic1+","+pic2+","+pic3+","+pic4+"\n");
				writer.flush();
				writer.close();
			}
